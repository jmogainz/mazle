...
import type { AdventureFailRequest } from '@/lib/adventureApi';
import { failAdventureAttempt } from '@/lib/server/adventure';
import {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const body = await readAdventureJson<AdventureFailRequest>(request);
    const result = await failAdventureAttempt(me, body);
