...
import { recordAnalyticsShare } from '@/lib/server/analytics';
...
export async function POST(request: Request) {
  ...
  if (body.kind !== 'copy' && body.kind !== 'native') {
    return jsonError(400, 'INVALID_KIND', 'kind must be copy|native.');
  }
  ...
  let stored = false;
  try {
    stored = await recordAnalyticsShare({ date: body.date, playerId, userId }, { kind: body.kind });
  } catch {
    stored = false;
  }
