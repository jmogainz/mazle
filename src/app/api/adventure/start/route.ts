...
import type { AdventureStartRequest } from '@/lib/adventureApi';
import { startAdventureAttempt } from '@/lib/server/adventure';
import {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const body = await readAdventureJson<AdventureStartRequest>(request);
    const result = await startAdventureAttempt(me, body);
