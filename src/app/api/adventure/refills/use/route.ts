...
import type { AdventureUseRefillRequest } from '@/lib/adventureApi';
import { consumeAdventureRefill } from '@/lib/server/adventure';
import {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const body = await readAdventureJson<AdventureUseRefillRequest>(request);
    const result = await consumeAdventureRefill(me, body);
